...
public class OAuth2Authorizer : OAuth2AuthorizerBase, IOAuth2Authorizer
{
    ...
    /// <exception cref="BlueSkyQueryException">When state passed here doesn't equal state passed to <see cref="BeginAuthorizeAsync(string?)"/>, then possible CSRF attack.</exception>
    public async Task CompleteAuthorizeAsync(string code, string? state = null)
    {
        ...
        if (expectedState != returnedState)
            throw new BlueSkyQueryException("Possible CSRF attack - State doesn't match. Check that the state sent in the begin request matches the state in the received request.");
