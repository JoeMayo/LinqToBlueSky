...
/// </summary>
internal partial class BlueSkyExecute : IBlueSkyExecute, IDisposable
{
    internal const int DefaultReadWriteTimeout = 300000;
    internal const int DefaultTimeout = 100000;

    /// <summary>
    /// Gets or sets the object that can send authorized requests to BlueSky.
    /// </summary>
    ...
    /// <summary>
    /// Assign your TextWriter instance to receive LINQ to BlueSky output
    /// </summary>
    ...
    /// supports testing
    /// </summary>
    public BlueSkyExecute(IAuthorizer authorizer)
    {
        Authorizer = authorizer ?? throw new ArgumentNullException("authorizedClient");
        Authorizer.UserAgent = Authorizer.UserAgent ?? L2BSKeys.DefaultUserAgent;
    }

    /// <summary>
    /// Used in queries to read information from BlueSky API endpoints.
    /// </summary>
    ...
    /// <returns>XML Respose from BlueSky</returns>
    public async Task<string> QueryBlueSkyAsync<T>(Request request, IRequestProcessor<T> reqProc)
    {
        WriteLog(request.FullUrl, nameof(QueryBlueSkyAsync));
    ...
    // TODO: Seems to be Twitter specific and might not translate to BlueSky
    /// <summary>
    /// Performs a query on the BlueSky Stream.
    /// </summary>
    /// <param name="request">Request with url endpoint and all query parameters.</param>
    /// <returns>
    ...
    /// </returns>
    public async Task<string> QueryBlueSkyStreamAsync(Request request)
    {
        //WriteLog(request.FullUrl, nameof(QueryBlueSkyStreamAsync));
        //_ = Authorizer ?? throw new ArgumentNullException(nameof(Authorizer), $"{nameof(Authorizer)} is required.");
        ...
        //await BlueSkyErrorHandler.ThrowIfErrorAsync(response).ConfigureAwait(false);
        ...
        //            byte[] postBytes = new byte[byteCount];

        //            memStr.Position = 0;
        //            await memStr.ReadAsync(postBytes.AsMemory(0, byteCount), CancellationToken).ConfigureAwait(false);

        //            string post = Encoding.UTF8.GetString(postBytes, 0, byteCount);
        //            var strmContent = new StreamContent(this, post);
    ...
    /// See this post on the BlueSky forums: https://twittercommunity.com/t/media-category-values/64781/6
    /// </param>
    ...
    public async Task<string> SendJsonToBlueSkyAsync<T>(string method, string url, IDictionary<string, string> postData, T postObj, CancellationToken cancelToken)
    {
        WriteLog(url, nameof(SendJsonToBlueSkyAsync));
    ...
    /// <param name="getResult">callback for handling async Json response - null if synchronous</param>
    /// <returns>Json Response from BlueSky - empty string if async</returns>
    public async Task<string> PostFormUrlEncodedToBlueSkyAsync<T>(string method, string url, IDictionary<string, string?> postData, CancellationToken cancelToken)
    {
        WriteLog(url, nameof(PostFormUrlEncodedToBlueSkyAsync));
